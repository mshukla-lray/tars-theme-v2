$(document).ready(function(e) {
	//Skip reading portlet content viewer HTML content by read speaker API.
	$(".asa.portlet").addClass("rs_skip");
	$(".asa.portlet.title").addClass("rs_skip");

/**changes to make the captcha challenge position fixed on button click**/
$(".widget-submit").click(function(){
	
	var iframeRcap = $(document).find('.goog-te-menu-frame');
	console.log("next = "+$(iframeRcap).next().children().length);
	console.log("next = "+$(iframeRcap).next().next().children().length);
	if($(iframeRcap).next().children().length>1){
		$(iframeRcap).next().children().css('position','fixed');
	}else{
	$(iframeRcap).next().next().children().css('position','fixed');
	}
});
});
//Home page revamp
...
$(function () {
  $(".search").click(function () {
    if ($(this).hasClass('active')) {
      $(this).find('span').text('');
      //$(this).find('img').attr('src', '/wps/contenthandler/dav/fs-type1/themes/RTA.Responsive.Theme/rta-assets/svg/revamp_sprite.svg#close');
    } else {
      $(this).find('span').text('');
  ...
  var SupportMenu =
    ...
    function () {
      function SupportMenu() {
        ...
        $('.supportMenuClose, .md-overlay, .support-menu__item').on('click', this.menuRemoveClass);
...
var onLoadCaptcha = function() {
	//console.log("hidden captcha load ....");
	$(".hidden-captcha-btn").each(function() {			
		var btnObject = $(this);
