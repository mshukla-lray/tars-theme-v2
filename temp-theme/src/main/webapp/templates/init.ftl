...
<#assign
	...
	root_folder = theme_display.getPathThemeRoot()
...
<#assign tarsIdentityProvider = propsUtil.get("tars.application.default.identity.provider")/>
<#assign tarsIntIdentityProvider = propsUtil.get("tars.int.openid.connect.provider.name")/>
<#assign tarsExtIdentityProvider = propsUtil.get("tars.ext.openid.connect.provider.name")/>
<#assign 
	plid = page.getPlid()
	logOutActionURL = portletURLFactory.create(request,"com_tars_logout_action_tarslogoutaction",plid,"ACTION_PHASE")
	logOutActionURL = logOutActionURL + "&p_p_state=normal&p_p_mode=view&_com_tars_logout_action_tarslogoutaction_javax.portlet.action=%2Ftars%2Flogout"
	
	log_in_url = "/home/-/login/openid_connect_request?p_p_state=maximized&_com_liferay_login_web_portlet_LoginPortlet_saveLastPath=false&_com_liferay_login_web_portlet_LoginPortlet_redirect=/&_com_liferay_login_web_portlet_LoginPortlet_OPEN_ID_CONNECT_PROVIDER_NAME=${tarsExtIdentityProvider}"
	log_in_int_url = "/home/-/login/openid_connect_request?p_p_state=maximized&_com_liferay_login_web_portlet_LoginPortlet_saveLastPath=false&_com_liferay_login_web_portlet_LoginPortlet_redirect=/&_com_liferay_login_web_portlet_LoginPortlet_OPEN_ID_CONNECT_PROVIDER_NAME=${tarsIntIdentityProvider}"
/>

<#if theme_display.getLanguageId() == "en_US">
	<#assign rtaLinkURL = "https://www.rta.ae/wps/portal/rta/ae/home?lang=en"/>
<#else>
	<#assign rtaLinkURL = "https://www.rta.ae/wps/portal/rta/ae/home?lang=ar"/>
</#if>

<#if theme_display.getLayout().isPrivateLayout()>
	<#assign pageURLWithOutParameters = "/group"+theme_display.getScopeGroup().getFriendlyURL()+ theme_display.getLayout().getFriendlyURL()/>
<#else>
	<#assign pageURLWithOutParameters = "/web"+theme_display.getScopeGroup().getFriendlyURL()+ theme_display.getLayout().getFriendlyURL()/>
</#if>

<#if theme_display.getLayout().isPrivateLayout()>
	<#assign globalSearchPageURL = "/group"+theme_display.getScopeGroup().getFriendlyURL()+ "/search"/>
<#else>
	<#assign globalSearchPageURL = "/web"+theme_display.getScopeGroup().getFriendlyURL()+ "/search"/>
</#if>

<#assign isInternalUser = "false"/>

<#if is_signed_in>
	<#assign screenName = user.getScreenName()?contains("lamd:") />
	<#assign internalUser = screenName?string("true", "false") />
	<#if internalUser == "true">
		<#assign isInternalUser = "true"/>
	</#if>
</#if>
